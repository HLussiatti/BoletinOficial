...
Está autorizado utilizar una API de modelos u otras herramientas que resulten adecuadas; no se exige ejecutar un modelo local. La elección técnica se hará durante el desarrollo según calidad en español, fidelidad, costo y tiempo, sin quedar ligada a BART ni a los experimentos anteriores. Se prevé usar un servicio por API como opción inicial, con proveedor y modelo configurables, reutilización de resúmenes ya calculados y registro de consumo. La disponibilidad de la cuenta y credencial necesarias se resolverá al integrar el servicio. Antes del procesamiento histórico masivo se medirá una muestra y se estimará el consumo total; se aplicarán límites configurables y reanudación por lotes.

El correo propuesto tendrá fecha, fuentes y período efectivamente consultados, cantidad de novedades y una ficha por norma. Adjuntará los PDF, sus anexos disponibles y un archivo de resúmenes, además de incluir una versión legible de las fichas en el cuerpo. El tamaño total se medirá antes del envío; si excede el límite del proveedor, se dividirá el boletín en correos numerados, manteniendo la correspondencia entre fichas y adjuntos.

El usuario dispone de correo institucional instalado localmente. Al implementar se identificará el cliente, su versión, el tipo de cuenta y el mecanismo admitido por la organización. No se presupone que sea Outlook ni que la instalación local habilite por sí sola el envío desatendido. Se elegirá la integración compatible con esa cuenta: API del proveedor, SMTP autenticado o automatización del cliente cuando permita operar de forma estable en las condiciones de esta máquina.

Se probará el envío con la sesión bloqueada y se documentará si requiere usuario conectado o cliente abierto. Esta decisión también formará parte de la configuración del instalador. Los destinatarios se administrarán mediante una lista de distribución o configuración explícita, comenzando por un buzón de prueba. Las credenciales estarán fuera del repositorio y del instalador, almacenadas mediante el mecanismo seguro disponible en el equipo.

La entrega se registrará por lote y por destinatario cuando corresponda. Si el proveedor rechaza el envío, se reintentará de forma acotada. Si acepta el correo pero la conexión se pierde antes de confirmar localmente el resultado, quedará como incierto y se buscará conciliación con el proveedor antes de reenviar. SMTP por sí solo no permite prometer ausencia absoluta de duplicados ante esa situación.
...
| 5. Integrar correo y operación temprana | 2–3 días hábiles | Identificar el correo institucional e integrar envío desatendido, adjuntos, límites de tamaño, registro de entrega y programación a las 05:30. | Validar el buzón de prueba, la ejecución con sesión bloqueada y la recuperación; medir el tiempo del ciclo diario respecto de las 06:00. |
...
| Correo | Cuenta institucional instalada localmente. Identificar cliente, proveedor, mecanismo de autenticación, remitente, destinatarios y buzón de prueba. | Cuenta confirmada; datos de integración por completar. |
...
| Servicio de resumen | Se permite API y la elección técnica de herramientas; no se exige modelo local. Obtener cuenta/credencial y configurar límites de consumo. | Libertad técnica confirmada; integración por completar. |
